 ...
 * @Description
 */
@FeignClient(name = "enerbos-eam-microservice", url = "${eam.microservice.url:}", fallbackFactory = PatrolCommonClientFallback.class)
public interface PatrolCommonClient {
...
@Component
class PatrolCommonClientFallback implements FallbackFactory<PatrolCommonClient> {

    @Override
    public PatrolCommonClient create(Throwable throwable) {
        return new PatrolCommonClient() {
            @Override
